...
const DetailsBannerForms = ({ bannerId }) => {
  ...
  //   const url = `https://7gardoon-servers.liara.run/api/updatePost/${bannerId}`;
  ...
  const remover = () => {
    const url = `https://7gardoon-servers.liara.run/api/deletePost/${bannerId}`;
  ...
  useEffect(() => {
    axios
      .get(`https://7gardoon-servers.liara.run/api/postById/${bannerId}`)
      .then((d) => {
        setFullData(d.data);
        setTag(d.data.tags);
